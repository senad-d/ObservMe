...
The dedicated `observme-llm-conversations.json` dashboard is the canonical opt-in content view. It shows a redacted conversation timeline, prompt/response/thinking panels, and trace links. Its Loki template filters cover `pi_session_id`, `pi_workflow_id` where promoted, `pi_agent_id`, `pi_agent_run_id`, provider, model, and content kind. The pinned Grafana 11.1/Loki 3.0 local stack uses supported `label_values(...)` variables only; it does not use Loki-incompatible `query_result(...)` variables.

Agent-name filtering is a bounded two-step interaction. The local Loki configuration indexes `pi_agent_display_name` only for the name lookup, and the dependent **Agent ID** variable resolves the selected presentation name to indexed `pi_agent_id` values. Conversation panels continue to filter only by the technical ID. Selecting a name alone only bounds the Agent ID choices; select the intended Agent ID to apply the filter. Duplicate friendly names therefore expose every matching ID instead of silently choosing or merging one. With both variables at **All**, Agent ID uses an explicit `.*` value so initial dashboard rendering cannot collapse to the empty regex `()`, panel queries contain no display-name matcher, and historical records without `pi_agent_display_name` remain visible.

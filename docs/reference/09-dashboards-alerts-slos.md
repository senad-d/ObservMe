...
The dedicated `observme-llm-conversations.json` dashboard is the canonical opt-in content view. It shows a redacted conversation timeline, prompt/response/thinking panels, and trace links. Session, workflow, agent name/ID, run, provider, and model variables discover values from both LLM lifecycle and captured-content events, so those context choices remain available when optional content capture is disabled. Content-kind choices remain capture-only. The pinned Grafana 11.1/Loki 3.0 local stack uses supported `label_values(...)` variables only; it does not use Loki-incompatible `query_result(...)` variables.

Agent-name filtering uses the indexed `pi_agent_display_name` label. Selecting **Agent name** filters conversation panels immediately and also bounds the dependent **Agent ID** choices. Duplicate friendly names intentionally show the combined records and expose every matching technical ID; select one exact Agent ID to disambiguate. With both variables at **All**, their explicit `.*` values prevent initial dashboard rendering from collapsing to the empty regex `()` and keep historical records without `pi_agent_display_name` visible.

This Loki-only promotion is an explicit cardinality decision: a valid agent ID has one launcher-supplied display name, so adding the functionally dependent name does not split that agent's existing ID-indexed streams further. Deployments that allow a display name to change for the same agent ID must not copy this promotion without accepting the additional streams. The metrics pipeline still deletes both display names and agent IDs before Prometheus resource-to-label conversion.

Keep a dashboard time range that includes newly emitted records. An empty range response is not a query error: if LLM lifecycle events and context-variable choices are also absent, widen the range and verify current producer/export health; if context choices exist but content and content-kind values do not, check opt-in capture with `/obs status`. Use `/obs health` for Collector and datasource reachability. Reset variables after a query error, and narrow the range plus select Session ID or Agent ID after a datasource timeout. Native Loki OTLP ingestion stores the captured body as the log line, so conversation panels must not apply `| json`; doing so attaches `JSONParserErr` to valid plain-text captured records.

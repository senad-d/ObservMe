...
On the first trusted `session_start` for a project, ObservMe creates `observme.yaml` under Pi's exported project config directory (`.pi/observme.yaml` in the standard distribution) when the file is missing. Every generated setting is commented out, so the untouched guide does not override active global defaults. Uncomment `observme:`, the required parent sections, and only the project settings you intend to adopt; the suggested profile keeps raw prompt, response, thinking, tool, bash, and path capture disabled.
...
| `/obs health` | Checks every enabled effective OTLP signal destination, Grafana, and configured datasource reachability with bounded timeouts and exporter-aligned transport policy. | Collector/Grafana |

...
- **Recovery on startup in an existing session:** read the current session header plus the latest validated `observme.correlation` custom entry on the active branch, set `pi.session.id`/`pi.agent.id`/resource attributes, and never continuously tail the session file. Historical telemetry is exported only through the explicit, confirmed `/obs backfill` command and is marked `observme.replayed=true`; automatic startup replay is unsupported.
